...
NATEX_ROOT=${NATEX_ROOT:-$(git rev-parse --show-toplevel)}
...
LOCAL_CI_TMP=${LOCAL_CI_TMP:-${NATEX_ROOT}/.tmp/local_ci_tmp/${CI_ARCH}}
...
for STAGE in "${STAGES[@]}"; do
    ...
    cp ${NATEX_ROOT}/ci/scripts/bootstrap_local_ci.sh ${LOCAL_CI_TMP}
    ...
    if [[ "${USE_HOST_GIT}" == "1" ]]; then
        DOCKER_RUN_ARGS="${DOCKER_RUN_ARGS} -v ${NATEX_ROOT}:/nat-examples"
    fi
